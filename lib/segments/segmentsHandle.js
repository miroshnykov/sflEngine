...
const calculateSegments = async (req, res, params, type) => {
    try {
        ...
        input.advertiserId = params.advertiserInfo.advertiserId.toString()
        input.advertiserName = params.advertiserInfo.advertiserName
        if (params.debug) {
        ...
        if (fraudWebsiteTest.length !== 0 && segmentsResolve.length === 0) {

            let pattern = /^https?\:\/\/www.|^www.?|^https?\:\/\/|^www.?|\.([^.]*)$/g
            let rSite = fraudWebsiteTest.filter(i => (
                params.refererDomain.toLowerCase().includes(i.value.replace(pattern, '').toLowerCase())
            ))
            if (rSite.length !== 0) {
