...
const calculateSegments = async (req, res, params, type) => {
    try {
        ...
        if (fraudWebsiteTest.length !== 0 && segmentsResolve.length === 0 && params.referer !== '') {

            let rSite = fraudWebsiteTest.filter(i => (
                params.referer.toLowerCase().includes(i.value.replace(pattern, '').toLowerCase())
            ))
            if (rSite.length !== 0) {
        ...
        if (fraudWebsites.length !== 0 && segmentsResolve.length === 0 && params.referer !== '') {
            if (params.debug) {
        ...
        if (segmentsResolve.length === 0) {
            ...
            let belongsSegmentIds = generalSegments.filter(i => {
                return (i.dimension === `affiliate` && i.value === input.affiliateId)
                    ...
                    || (i.dimension === `website` && input.website.toLowerCase().includes(i.value.replace(pattern, '').toLowerCase()))
                    || i.include
            ...
            if (params.debug) {
                ...
                params[`${type}SEGMENTS_BEFORE_PROCESSING`] = generalSegments
        ...
        if (segmentsResolve.length !== 0) {
            ...
            // console.log('affInfo:', params.affInfo)
