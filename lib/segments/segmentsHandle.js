...
const calculateSegments = async (req, res, params, type) => {
    try {
        ...
        if (segmentsResolve.length === 0) {
            ...
            if (params.debug) {
                ...
                if (generalSegments.length < 250) {
                    ...
                    params[`${type}SEGMENTS_BEFORE_PROCESSING_COUNT`] = generalSegments.length
                }
        ...
        if (segmentsResolve.length !== 0) {
            ...
            if (!resolveLPIndex.staticUrl) {
                let randomSites = await getRandomSitesEvent()
                resolveLPIndex.staticUrl = pickRandomSites(randomSites)
                resolveLPIndex.name = 'randomSites'

            }
            lidForSegment['landing_page'] = resolveLPIndex.name || ''
            lidForSegment['landing_page_id'] = resolveLPIndex.landingPageId || ''
            if (params.debug) {
