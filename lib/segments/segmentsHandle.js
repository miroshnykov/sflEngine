...
const calculateSegments = async (req, res, params) => {
    try {
        ...
        await segmentResolveProcessing(req, res, segmentsResolve, params)
...
const segmentResolveProcessing = async (req, res, segmentsResolve, params) => {
    if (segmentsResolve.length !== 0) {
        ...
        lidForSegment['affiliate_id'] = params.affiliateId && params.affiliateId.toString() || ''
        lidForSegment['campaignId'] = params.campaignId
        lidForSegment['campaign_id'] = params.campaignId && params.campaignId.toString() || ''
        lidForSegment['programId'] = params.programId
        lidForSegment['program_id'] = params.programId && params.programId.toString() || ''
        ...
        lidForSegment['account_executive_id'] = params.affInfo && params.affInfo.accountManagerId || 0
...
const overrideProduct = async (segmentsResolve, lidForSegment, params, resolveLP) => {
    ...
    lidForSegment['inputProgram'] = params.programId
    lidForSegment['outputProduct'] = resolveLP.productId
    lidForSegment['productId'] = resolveLP.productId
    lidForSegment['product_id'] = resolveLP.productId && resolveLP.productId.toString() || ''
    lidForSegment['programId'] = resolveLP.programId
    lidForSegment['program_id'] = resolveLP.programId && resolveLP.programId.toString() || ''

    let advertiserOverrideInfo = await getAdvertisersByProdIdEvent(resolveLP.productId)

    // console.log('advertiserOverrideInfo:', advertiserOverrideInfo)

    lidForSegment['inputAdvertisers'] = params.advertisers
    lidForSegment['inputAdvertiserId'] = params.advertiser_id
    lidForSegment['inputAdvertiserName'] = params.advertiser_name
    lidForSegment['inputAdvertiserProductId'] = params.advertiser_product_id
    lidForSegment['inputAdvertiserProductName'] = params.advertiser_product_name

    params.advertiserInfo = advertiserOverrideInfo
    params.advertisers = advertiserOverrideInfo && advertiserOverrideInfo.advertiserId
