...
const logger = require('bunyan-loader')(config.log).child({scope: 'params.js'})


const {getDataCache, setDataCacheEx} = require('../cache/redis')
...
const getParams = async (req) => {
    ...
    let redisKeyRefCode = `${apiInputData.ref}-${apiInputData.prod }`
    let cacheData = await getDataCache(redisKeyRefCode)

    let refInfoNew
    if (cacheData) {
        logger.info(` *** GET RefCodeInfo from cacheData:${JSON.stringify(cacheData)}`)
        metrics.influxdb(200, `refCodeFromCache`)
        refInfoNew = cacheData

    } else {
        refInfoNew = await getRefCodeInfo_(apiInputData)
        if (refInfoNew) {
            metrics.influxdb(200, `refCodeFromSflApi`)
            await setDataCacheEx(redisKeyRefCode, refInfoNew)
        }
    }

    if (!refInfoNew) {
        refInfoNew = config.refCodeDefault
        refInfoNew.productId = apiInputData.prod || 0
        logger.info(` **** getRefCodeInfo no data, *** Input data:${JSON.stringify(apiInputData)} IP:${params.ip}`)
        metrics.influxdb(500, `setRefCodeDefault`)
    }

    if (!Number(refInfoNew.programId) || !Number(refInfoNew.productId)) {
        logger.info(` **** programIdOrProductIdIsNull refInfoNew:${JSON.stringify(refInfoNew)}, *** Input data:${JSON.stringify(apiInputData)}`)
        metrics.influxdb(500, `programIdOrProductIdIsNull`)
        refInfoNew = config.refCodeDefault
    }
    // logger.info('refInfoNew:',refInfoNew)
    ...
    //     if (!affInfo) {
    //         logger.info(`RedisEmptyAffiliateId-${affiliateId}`)
