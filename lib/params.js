...
const getParams = async (req) => {
    ...
    // console.log('redisKeyRefCode:',redisKeyRefCode)
    let cacheData = await getDataCache(redisKeyRefCode)

    let refInfoNew
    if (cacheData) {
        console.log(` *** GET RefCodeInfo from cacheData:${JSON.stringify(cacheData)}`)
    ...
    if (!refInfoNew) {
        ...
        console.log(` **** getRefCodeInfo no data, *** Input data:${JSON.stringify(apiInputData)} IP:${params.ip}`)
        metrics.influxdb(500, `setRefCodeDefault`)
    }

    if (!Number(refInfoNew.programId) || !Number(refInfoNew.productId)) {
        console.log(` **** programIdOrProductIdIsNull refInfoNew:${JSON.stringify(refInfoNew)}, *** Input data:${JSON.stringify(apiInputData)}`)
