...
const getLid = async (lid) => {
    ...
    try {
        ...
        leadInfo.TableName = config.aws.dynamodb.tableName


        let dynamoDbConf =  {}
        dynamoDbConf.region = config.aws.dynamodb.region
        dynamoDbConf.endpoint = config.aws.dynamodb.endpoint
        dynamoDbConf.accessKeyId = config.aws.access_key
        dynamoDbConf.secretAccessKey = config.aws.secret_key
        dynamoDbConf.tableName = config.aws.dynamodb.tableName
...
const createLid = async (lidInfo) => {
    ...
    try {
        ...
        let leadParams = {}
        leadParams.TableName = config.aws.dynamodb.tableName
...
const redshiftOffer = (lidObj) => (

    {
        ...
        'advertiser': lidObj.advertiser || '',
        'verticals': lidObj.verticals || '',
...
const redshiftAggrStats = (lidObj) => (

    {
        ...
        'event_type': 'click',
        'event_count': 1,
        'account_executive_id': lidObj.accountManagerId || 0,
        'account_manager_id': lidObj.accountExecutiveId || 0,
        'affiliate_type': lidObj.affiliateType || '',
        'referer_domain': lidObj.adDomain || '',
        'referer_path': `${lidObj.adPath}||debug||segmentType-${lidObj.segmentType}`,
