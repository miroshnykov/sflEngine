...
const createLidAndSendAggrStats = async (lidInfo) => {
    ...
    try {
        ...
        let affInfo = await getDataCache(`affiliate-${lidInfo.affiliateId}`)

        let stats = redshiftAggrStats(lidInfo, affInfo)
        // console.log(stats)
        process.send({
            type: 'clickAggrStats',
            value: 1,
...
const redshiftAggrStats = (lidObj, affInfo) => (

    {
        ...
        'event_type': 'click',
        'event_count': 1,
        'account_executive_id': affInfo && affInfo.accountManagerId || 0,
        'account_manager_id': affInfo && affInfo.accountExecutiveId || 0,
        'affiliate_type': affInfo && affInfo.affiliateType || 0,
        'referer_domain': lidObj.adDomain || '',
        'referer_path': lidObj.adPath + '-debug' || 'debug',
