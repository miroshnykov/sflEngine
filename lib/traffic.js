...
const {getOffer, getCampaign} = require('../cache/local/offers')
...
let traffic = {
    ...
    ad: async (req, res, next) => {
        try {
            ...
            try {
                ...
                let offerInfo = await getOffer(decodedObj.offerId) || []
                response.offerInfo = offerInfo
                console.log('offerInfo:', offerInfo)

                let campaignInfo = await getCampaign(decodedObj.campaignId) || []
            ...
            // params.lid = v4()
            // let lidObj = generateLid(req, {}, params)
            // createLid(lidObj)
            endTime = new Date() - startTime
    ...
    getDataCache: async (req, res, next) => {
        try {
            ...
            if (offerId) {
                let offerInfo = await getOffer(offerId) || []
                response.offerInfo = offerInfo
            }

            if (campaignId) {
                let campaignInfo = await getCampaign(campaignId) || []
                response.campaignInfo = campaignInfo
