...
let traffic = {
    signup: async (req, res, next) => {
        try {
            ...
            // response.ipInfo1 = req.headers['x-forwarded-for'] || req.connection.remoteAddress ||  req.socket.remoteAddress || req.connection.socket.remoteAddress
            // response.ipInfo2 = req.headers['x-forwarded-for'] || req.connection.remoteAddress
            // response.reqIp = req.ip
            // response.headers = req.headers
            let params = getParams(req)
            console.log(`\n\x1b[31m**** Start signup:\x1b[0m   `)
            // let findUnderLimit = targeting.filter(item => (
            ...
            if (find.length === 0) {
                ...
                console.log(` Stop signup REDIRECT TO FR FRDetails ${JSON.stringify(response)} \n`)
                // res.send(response)
            ...
            metrics.setStartMetric({
                route: 'redirectToSflLandingPage',
                method: 'GET'
            })
            console.log(`redirectToSflLandingPage metrics `)
            ...
            let redirectToLPMessage = `TRAFFIC, REDIRECT TO Landing page: { ${redirectToLP} } `
            response.redirectToLP = redirectToLPMessage
            // console.log(redirectToLPMessage)
            console.log(` Stop signup LID { ${params.lid} }, details: ${JSON.stringify(response)} \n`)
