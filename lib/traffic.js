...
const {getBlockSegmentsLocal, getLandingPagesLocal} = require('../cache/local/blockSegments')
...
const {resolveBlockSegments} = require('./resloveBlockSegments')
...
let traffic = {
    signup: async (req, res, next) => {
        try {
            ...
            params.affiliate_country = `${affiliate_id}/${params.country}`
            let blockSegmentsResolve = await resolveBlockSegments(params, blockSegments)
            ...
            if (blockSegmentsResolve.length !== 0) {
                ...
                response.blockSegmentsResolve = blockSegmentsResolve[0]
    ...
    ad: async (req, res, next) => {
        try {
            ...
            if (offerInfo.customLpRules) {
                response.customLpRulesExists = '*************** customLpRules ************* '
                let customLpRules = parseJson(offerInfo.customLpRules)
                response.customLpRules = customLpRules
                ...
                response.customLpRules = 'There is no customLpRules  set up'
            }
            ...
            if (offerInfo.geoOfferId) {
                ...
                response.resolveGeo = 'There is no GEO restriction set up'
            }
            ...
            if (campaignInfo.targetRules) {
                ...
                response.resolveCampaignRules = 'There is no campaign targetRules'
            }
