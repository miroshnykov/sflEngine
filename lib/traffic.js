    ...
    signup: async (req, res, next) => {
        try {
            ...
            if (find.length === 0) {
                ...
                let redirectMessage = `\n      ***** STOP TRAFFIC, REDIRECT TO FR:${fr} `
                console.log(redirectMessage)
                response.FR = redirectMessage
                console.log(` \n      *****  Details ${JSON.stringify(response)} \n`)
                console.log(`\x1b[31m***** Stop signup:\x1b[0m `)
                // res.send(response)
            ...
            let redirectToLPMessage = `\n       ***** TRAFFIC, REDIRECT TO Landing page: { ${redirectToLP} } `
            response.redirectToLP = redirectToLPMessage
            console.log(redirectToLPMessage)
            console.log(` \n       ***** LID \x1b[33m{ ${params.lid} }\x1b[0m ${JSON.stringify(response)} \n`)
            console.log(`\x1b[31m***** Stop signup:\x1b[0m `)
