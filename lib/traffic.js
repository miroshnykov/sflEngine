...
const {targetingConditions} = require('../cache/local/getTargeting')
...
const {getBudgetCampaign, addClick} = require('../cache/api/getBudget')
...
let traffic = {
    signup: async (req, res, next) => {
        try {
            ...
            let spent = await getBudgetCampaign(find[0].campaignId)
            let budgetDaily = Number(find[0].budgetDaily)
            let budgetTotal = Number(find[0].budgetTotal)
            let dailySpentCalculate = Number(find[0].campaignCpc) * Number(spent.dailySpent)
            let totalSpentCalculate = Number(find[0].campaignCpc) * Number(spent.totalSpent)
