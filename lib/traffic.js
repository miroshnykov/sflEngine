    ...
    signup: async (req, res, next) => {
        try {
            ...
            if (find.length === 0) {
                ...
                console.log('rangeSpeed(endTime):', rangeSpeed(endTime))
                metrics.setStartMetric({
                    route: `FR-speed-${rangeSpeed(endTime)}`,
                    method: 'GET'
                })
                metrics.sendMetricsRequest(300)
            ...
            const {affiliate_id, campaign_id, program_id} = await createLid(lidObj)
            ...
            endTime = new Date() - startTime
            metrics.setStartMetric({
                route: `SFL-speed-${rangeSpeed(endTime)}`,
                method: 'GET'
            })
            metrics.sendMetricsRequest(300)
